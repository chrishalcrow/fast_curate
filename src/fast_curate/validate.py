...
class ValidateWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, labels, output_folder, analyzers, local_model, repo_model=None):
        ...
        current_model = local_model if local_model is not None else repo_model
        which_model_text = QtWidgets.QTextEdit(f"Labels generated by model at <strong>{current_model}</strong>")
        which_model_text.setReadOnly(True)
        which_model_text.setStyleSheet("background-color: white")
        validateLayout.addWidget(which_model_text, self.validateLayout_counter , 0)
        ...
        validateLayout.setRowStretch(0, 1)
        validateLayout.setRowStretch(1, 8)
        validateLayout.setRowStretch(2, 1)
