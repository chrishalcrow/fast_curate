...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def initialise_plot(self):
        ...
        self.correlogram_widget.setLabels(title="Auto-correlogram", bottom="time (ms)", left="count")
        

        self.correlogram_zoom_plot = self.correlogram_zoom_widget.plot(
            stepMode="left", fillLevel=0, fillOutline=True, brush=color_1)
        self.correlogram_zoom_widget.setLabels(title="Auto-correlograms (zoom out)", bottom="time (ms)", left="count")
        

        self.max_templates_plot = self.max_template_widget.plot(
            pen=pg.mkPen(color_3, width=3))
        self.max_template_widget.setLabels(title="Unit template on max channel", bottom="time (ms)", left="Signal (uV)")


        self.amps_raster_plot = self.amp_raster_widget.plot(
            pen=None, symbolPen=None, symbol="o", symbolBrush=color_3, symbolSize=4)
        self.amp_raster_widget.setLabels(title="Amplitude of spikes on max template channel", bottom="time (s)", left="Amplitude (uV)")


        self.locs_raster_plot = self.loc_raster_widget.plot(
            pen=None, symbolPen=None, symbol="o", symbolBrush=color_2, symbolSize=4)
        self.loc_raster_widget.setLabels(title="Location of spikes in time", bottom="time (s)", left="y-Location (um)")
        

        self.spike_locs_plot = self.spike_locs_widget.plot(
            pen=None, symbolPen=None, symbol="o", symbolBrush=color_2, symbolSize=4)
        self.spike_locs_widget.setLabels(title="Location of spikes in space", bottom="x (um)", left="y (um)")


        self.binned_spikes_plot = self.binned_spikes_widget.plot(
            stepMode="left", fillLevel=0, fillOutline=True, brush=color_1)
        self.binned_spikes_widget.setLabels(title="Binned spike counts", bottom=f"Bin number. Bin width = ", left="counts")
    ...
    def save_labels(self):
        ...
        save_choice_df = pd.read_csv(self.output_folder / Path("decision_data_cache.csv"))
        ...
        final_choices_df = final_choices_df.rename(columns={'keystroke': 'label'})
        
        final_choices_df.to_csv(self.output_folder / Path("decision_data_with_metics.csv"), index=False)
        just_labels = final_choices_df[['unit_id', 'label']]
        just_labels.to_csv(self.output_folder / Path("just_labels.csv"), index=False)
